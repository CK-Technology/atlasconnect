# AtlasConnect Server Dockerfile
...
COPY --from=builder /app/server/target/release/atlasconnect-server /app/
...
# Expose port 8443 (AtlasConnect server)
EXPOSE 8443

# Start the server
ENTRYPOINT ["./atlasconnect-server"]
